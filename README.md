...
nix eval --raw 'nixpkgs#stellarium' | xargs -o nix-tree --store https://cache.nixos.org

# Build in a temporary store and examine the output
nix build --store /tmp/temproot 'nixpkgs#hello' --print-out-paths | xargs -o nix-tree --store /tmp/temproot
```

For valid `--store` options, see [`nix help-stores`](https://nixos.org/manual/nix/stable/command-ref/new-cli/nix3-help-stores).
